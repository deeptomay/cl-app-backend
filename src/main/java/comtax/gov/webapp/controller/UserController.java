...
import comtax.gov.webapp.service.UserProjectService;
...
public class UserController {
	...
	private final UserProjectService  projectroleservice; 
...
//	}
	...
	@GetMapping("/assigned/{postingType}")
	public ResponseEntity<ApiResponse<List<UserAssignDet>>> getAssignUsers(@AuthenticationPrincipal Jwt jwt,
			HttpServletRequest request,@PathVariable String postingType) {
		log.info("Fetching assigned users for approver getAssignUsers: {}", jwt.getSubject());
		List<String> roles = jwt.getClaimAsStringList("roles").stream()
                .map(r -> r.replaceFirst("ROLE_", ""))
                .collect(Collectors.toList());   
		String firstRole = roles.isEmpty() ? null : roles.get(0);
		try {
			List<UserAssignDet> assignedUsers = assignService.getAllUsersWithPostingsAndProjects(jwt.getSubject(),firstRole,postingType);
			log.info("assignedUsers.size()"+assignedUsers.size());
			for(int i=0;i<assignedUsers.size();i++) {
				log.info("assigned user list>>> Postings->"+assignedUsers.get(i).getPostings());
			}
			return buildListResponse(assignedUsers, "Assigned users fetched successfully", "No assigned users found",
					request);
		} catch (Exception ex) {
			log.error("Error fetching assigned users: {}", ex.getMessage(), ex);
			throw new ServiceException("Failed to fetch assigned users", ex);
		}
	}
	...
	public ResponseEntity<ApiResponse<String>> add_module(@Valid @RequestBody AddModuleRequest req,
		...
		try {
			...
			log.error("Unexpected error during add module {}: {}",  ex.getMessage(), ex);
	...
	public ResponseEntity<ApiResponse<String>> update_module(@Valid @RequestBody AddModuleRequest req,
			@AuthenticationPrincipal Jwt jwt, HttpServletRequest request) {
		log.info("Received assignment request: {} by user {}", req, jwt.getSubject());
		try {
			assignService.updateModule(req);
			return buildSuccessResponse("Project edited Successfully", jwt.getSubject(), request);
		} catch (ServiceException se) {
			log.error("Service error during Edit Module {}: {}",  se.getMessage(), se);
			throw se;
			
		} catch (Exception ex) {
			log.error("Unexpected error during Edit module {}: {}",  ex.getMessage(), ex);
			throw new ServiceException("Unexpected error during Edit Module " , ex);
			
		}
	}
	
	
	@GetMapping("/get-postings")
	public ResponseEntity<ApiResponse<List<UserAssignDet>>> getUserPostingForSidebar(@AuthenticationPrincipal Jwt jwt,
			HttpServletRequest request) {
		
		String hrms = jwt.getSubject();
		log.info("Fetching user posting details in sidebar display section for HRMS: {}", hrms);
		return buildListResponse(assignService.fetchPostingNamesForSidebar(hrms), "Sidebar Posting details fetched successfully",
				"No posting details found for sidebar", request);
	}
	
	  @GetMapping("/get-role-module/{proj_id}")
	    public ResponseEntity<UserProjectRoleProjection> getRole(@AuthenticationPrincipal Jwt jwt,@PathVariable("proj_id") String proj_id,
				HttpServletRequest request
	           ) 
	  
	  {
		  ...
		  String hrms = jwt.getSubject();
		  log.info("proj id: {}", proj_id);

	        return ResponseEntity.ok(
	                projectroleservice.getRoleByHrmsAndProject(hrms, proj_id)
	        );
	    }
