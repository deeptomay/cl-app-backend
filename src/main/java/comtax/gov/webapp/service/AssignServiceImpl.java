...
public class AssignServiceImpl implements AssignService {
	...
	public boolean addModule(AddModuleRequest bn) {		
		...
		try {
			...
			if(!addmodule) {
				log.warn("Add Module Successfully");
				return false;
			}else {
				return true;
			}
				...
				log.error("Add Module Failed", e.getMessage());
				throw new RuntimeException(e.getMessage()); // Triggers rollback
			}
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public boolean updateModule(AddModuleRequest bn) {
		log.info("Starting Edit Module");
		try {
		
			boolean addmodule = assignEmpRepo.editModule(bn);
			if(!addmodule) {
				log.warn("Edit Module Successfully");
				return false;
			}else {
				return true;
			}
				...
				log.error("Edit Module Successfully", e.getMessage());
				throw new RuntimeException(e.getMessage()); // Triggers rollback
			}
	}
	...
	public List<UserAssignDet> fetchPostingNamesForSidebar(String hrms) {
		log.info("Fetching posting data for sidebar");
		return assignEmpRepo.fetchPostingNamesForSidebar(hrms);
	}
