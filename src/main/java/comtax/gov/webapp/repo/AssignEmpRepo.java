...
//	---------------------------21-01-2026--------------------
	...
	String SQL_FETCH_USERS_BASE_POSTINGS_ADL ="SELECT DISTINCT  um.hrms_code, um.full_name, um.email, d.designation AS "
			+ " desig_name, um.bo_id,upi.image_url AS profile_image_url FROM impact2_user_posting_det upd INNER JOIN "
			+ " impact2_user_master um ON upd.hrms_code = um.hrms_code LEFT JOIN designation_cd d ON "
			+ " um.desig_cd = d.desig_cd LEFT JOIN impact2_profile_image upi ON um.hrms_code = upi.hrms_code "
			+ " WHERE upd.status = 'L'   ORDER BY um.hrms_code";
...
//			+ "  WHERE upd.status = ? ";
	...
	String SQL_FETCH_POSTINGS = "SELECT upd.hrms_code,upd.posting_type,upd.office_type,upd.office_cd AS office_id,"
			...
			+ "  LEFT JOIN office_cd ofm ON upd.office_cd = ofm.office_cd AND upd.office_type = 'OF' "
			+ "  LEFT JOIN impact2_user_master emp ON upd.approver_hrms_code = emp.hrms_code "
			+ "  WHERE upd.status = ? and upd.posting_type= ? or upd.posting_type in (?,?) ";
	
//	--------------------------------------20-01-2026--------------------------------------------------------------------------
	String SQL_FETCH_POSTING_MY_ADDL="SELECT upd.hrms_code,upd.posting_type,upd.office_type,upd.office_cd AS office_id,\r\n"
			+ "			  CASE    WHEN upd.office_type = 'CI' THEN cim.circle_nm \r\n"
			+ "			  WHEN upd.office_type = 'CH' THEN chm.charge_nm    WHEN upd.office_type = 'OF' THEN ofm.office_nm \r\n"
			+ "			  ELSE NULL    END AS office_name, \r\n"
			+ "			  TO_CHAR(upd.active_dt, 'YYYY-MM-DD HH24:MI:SS') AS active_dt,   emp.full_name AS approver_name, \r\n"
			...
			+ "			  LEFT JOIN office_cd ofm ON upd.office_cd = ofm.office_cd AND upd.office_type = 'OF' \r\n"
			+ "			  LEFT JOIN impact2_user_master emp ON upd.approver_hrms_code = emp.hrms_code \r\n"
			+ "			  WHERE upd.status = ? and upd.approver_hrms_code=? and upd.inactive_dt is null and upd.posting_type= ? or upd.posting_type in (?,?)";
...
//	-----------------------------------------------------------20-01-2026------------------------------------------------------------------
	...
	String UPDATE_PROJECT = "update impact2_project_details "
			+ " set project_name=?, project_url=? "
			+ " where project_id =? ";
	
	String FETCH_POSTING_SIDEBAR = "select officeName from(select office_nm officeName from office_cd o,impact2_user_posting_det p where o.office_cd=p.office_cd and p.hrms_code=? "
			+ " union "
			+ " select circle_nm officeName from circle_cd o,impact2_user_posting_det p where o.circle_cd=p.office_cd and p.hrms_code=? "
			+ " union "
			+ " select charge_nm officeName from charge_cd o,impact2_user_posting_det p where o.charge_cd=p.office_cd and p.hrms_code=?)";
