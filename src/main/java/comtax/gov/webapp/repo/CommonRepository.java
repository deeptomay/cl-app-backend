...
//    String FETCH_USER_SQL="SELECT HRMS_CODE,FULL_NAME,EMAIL,PHONE_NO,DESIG_CD,GPF_NO,PAN_NO,BO_ID FROM  IMPACT2_USER_MASTER where USR_STATUS_CD=?";
	...
	String SELECT_USER_DET =
			"SELECT um.hrms_code AS hrmsCode, um.full_name AS fullName, um.email AS email, um.phone_no AS phoneNo, " +
			"d.designation AS desigName, um.gpf_no AS gpfNo, um.pan_no AS panNo, um.bo_id AS boId, " +
			"upi.image_url AS profileImageUrl, um.joining_order_no AS joiningOrderNo, um.blood_group AS bloodGroup, " +
			"um.house_no AS houseNo, um.police_station AS policeStation, um.district AS district, " +
			"um.state AS state, um.pin_code AS pinCode ,um.gender " +
			"FROM impact2_user_master um " +
			"LEFT JOIN designation_cd d ON um.desig_cd = d.desig_cd " +
			"LEFT JOIN impact2_profile_image upi ON um.hrms_code = upi.hrms_code " +
			"AND upi.image_upload_dt = (SELECT MAX(image_upload_dt) FROM impact2_profile_image WHERE hrms_code = um.hrms_code) " +
			"AND upi.status = 'L' " +
			"WHERE um.hrms_code = ?";
    ...
    String UPDATE_PROFILE_SQL = "UPDATE impact2_user_master SET joining_order_no = ?, blood_group = ?, email = ?, phone_no = ?, gpf_no = ?, bo_id = ?, house_no = ?, police_station = ?, district = ?, state = ?, pin_code = ? WHERE hrms_code = ?";
	...
	void updateUserProfile(String hrmsCode, UserDet userDet);
}
